...
class TestCostOfRefQty(TestCase):
    def test_result_is_correct(self):
        sc = fx.SupportsCostPerQuantityTestable(pref_unit='kg', ref_qty=2, cost_per_g=0.5)
...
class TestPersistableData(TestCase):
    def test_data_is_correct(self):
        # Check empty data;
        sc = fx.SupportsCostPerQuantityTestable()
        self.assertEqual(sc.persistable_data['cost_data'], model.cost.CostPerQtyData(
            ref_qty=100,
            pref_unit='g',
            cost_per_g=None
        ))
        # Check when defined;
        sc = fx.SupportsCostPerQuantityTestable(ref_qty=150, pref_unit='mg', cost_per_g=12)
        self.assertEqual(sc.persistable_data['cost_data'], model.cost.CostPerQtyData(
            ref_qty=150,
            pref_unit='mg',
            cost_per_g=12
