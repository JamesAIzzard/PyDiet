...
class TestConstructor(TestCase):
    def test_get_correct_instance(self):
        self.assertTrue(isinstance(model.quantity.IsSettableQuantityOf(
            qty_subject=mock.Mock()
        ), model.quantity.IsSettableQuantityOf))

    def test_loads_data_if_provided(self):
        sqo = model.quantity.IsSettableQuantityOf(
            qty_subject=mock.Mock(),
...
class TestResetPrefUnit(TestCase):
    def test_pref_unit_is_reset_correctly(self):
        sqo = model.quantity.IsSettableQuantityOf(qty_subject=mock.Mock())
...
class TestSetQuantity(TestCase):
    ...
    def test_raises_exception_if_vol_unit_used_but_not_configured(self):
        sqo = model.quantity.IsSettableQuantityOf(qty_subject=fx.HasReadableExtendedUnitsTestable(g_per_ml=None))
    ...
    def test_raises_exception_if_piece_unit_used_but_not_configured(self):
        sqo = model.quantity.IsSettableQuantityOf(qty_subject=fx.HasReadableExtendedUnitsTestable(piece_mass_g=None))
