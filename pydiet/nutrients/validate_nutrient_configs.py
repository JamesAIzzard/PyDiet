...
for name in nutrients.configs.nutrient_aliases.keys():
    if not name in apnn:
        raise nutrients.exceptions.NutrientConfigsError('{} is in nutrient_aliases.keys() but is not a primary nutrient name'.format(name))
...
for alias_list in nutrients.configs.nutrient_aliases.values():
    for name in alias_list:
        if name in apnn:
            raise nutrients.exceptions.NutrientConfigsError('{} is listed as an alias but is also a primary nutrient name'.format(name))            
...
for name in nutrients.configs.nutrient_group_definitions.keys():
    if not name in apnn:
        raise nutrients.exceptions.NutrientConfigsError('{} is a group definition key, but is not a primary nutrient name'.format(name))
...
for group_name in nutrients.configs.nutrient_group_definitions.keys():
    for name in nutrients.configs.nutrient_group_definitions[group_name]:
        if not name in apnn:
            raise nutrients.exceptions.NutrientConfigsError('{} is in included in the group {} but is not a primary nutrient name'.format(name, group_name))

# Check that nutrient_flag_rels.keys() are in all_flag_names;
for flag_name in nutrients.configs.nutrient_flag_rels.keys():
    if not flag_name in flags.configs.all_flag_names:
        raise nutrients.exceptions.NutrientConfigsError('{} is in nutrient_flag_rels but not in all_flag_names'.format(flag_name))
# Check that nutrient_flag_rels.values() are in all_primary_nutrient_names;
for flag_name in nutrients.configs.nutrient_flag_rels.keys():
    for name in nutrients.configs.nutrient_flag_rels[flag_name]:
        if not name in apnn:
            raise nutrients.exceptions.NutrientConfigsError('{} is in nutrient_flag_rels but is not a primary nutrient name'.format(name))            
...
for name in nutrients.configs.calorie_nutrients.keys():
    if not name in apnn:
        raise nutrients.exceptions.NutrientConfigsError('{} is in calorie_nutrients.keys() but is not a primary nutrient name'.format(name))        