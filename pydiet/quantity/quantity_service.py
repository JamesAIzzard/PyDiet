...
def units_are_masses(*units:str) -> bool:
    for unit in units:
        if not unit in get_recognised_mass_units():
            return False
    return True

def units_are_volumes(*units:str) -> bool:
    for unit in units:
        if not unit in get_recognised_vol_units():
            return False
    return True
...
def convert_qty_unit(qty: float,
    ...
    # mass<->vol
        ...
        if g_per_ml == None:
            raise ValueError('g_per_ml is required for mass<->vol conversions.')
    ...
    # pc<->mass
        ...
        if piece_mass_g == None:
            raise ValueError('piece_mass_g is required for pc<->mass conversions.')
    ...
    # pc->vol
        ...
        if piece_mass_g == None:
            raise ValueError('piece_mass_g is required for pc<->vol conversions.')
        if g_per_ml == None:
            raise ValueError('g_per_ml is required for pc<->vol conversions.')                   
        return _convert_pc_and_vol(qty, start_unit, end_unit, piece_mass_g, g_per_ml)

    else:
        raise ValueError('Unable to convert these units.')
