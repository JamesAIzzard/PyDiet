...
class IngredientEditService():
    ...
    def flag_number_name_map(self) -> Dict[int, str]:
        ...
        if not self._flag_number_name_map:
            self._flag_number_name_map = \
                self._create_number_name_map(list(self.ingredient.all_flag_data.keys()))
        # Return from cache;
    ...
    def _create_number_name_map(self, list_to_map: List) -> Dict[int, str]:
        map: Dict[int, str] = {}
        for i, key in enumerate(list_to_map, start=1):
            map[i] = key
        return map
