...
class EditIngredientNutrientMenuComponent(ConsoleAppComponent):
    ...
    def dynamic_response(self, response):
        ...
        number_map = self._ies.primary_nutrient_number_name_map.copy()
        number_map.update(self._ies.defined_secondary_nutrient_number_name_map.copy())
        if response in number_map.keys():
            if response in self._ies.primary_nutrient_number_name_map.keys():
                ...
                nutrient_name = self._ies.primary_nutrient_number_name_map[int(
                    response)]
            elif response in self._ies.defined_secondary_nutrient_number_name_map.keys():
                # Get the nutrient name;
                nutrient_name = self._ies.defined_secondary_nutrient_number_name_map[int(
                    response)]
            else:
                # Response not on the list for some reason;
                raise KeyError
